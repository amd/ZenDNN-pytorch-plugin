...
__The latest ZenDNN Plugin for PyTorch* (zentorch) 5.2 is here!__

zentorch 5.2 is the PyTorch plugin which comes with ZenDNN 5.2.
...
```
>Note: The repository defaults to the master branch. To build version 5.2, please check out the r5.2 branch; otherwise, it will build using the master branch.
```bash
git checkout r5.2
```
...
zentorch v5.2 is supported with ZenDNN v5.2. Please see the **Tuning Guidelines** section of ZenDNN User Guide for performance tuning. ZenDNN User Guide can be downloaded from [here](https://developer.amd.com/zendnn)
