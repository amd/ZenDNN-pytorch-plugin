...
An op at cpp level in _zentorch_ acts as a wrapper/bridge between torch data structures and their respective APIs and similarly between zendnn data structures and the corresponding APIs. Whenever we need to add a new op we need to follow the steps given below.
...
The op must be registered in the `src/cpu/python/zentorch/_meta_registrations.py` with the decorator @register_meta("{op_name}"). This registration has the function protoype and the corresponding output is returned with the appropriate shapes. This registration happens in pythonic way.
```python
...
# 6. Unit-testing
Unit tests for Python are located in a script `test_zentorch.py` inside the test directory. It contains tests for all ops supported by _zentorch_, bf16 device support check and a few other tests. The pre-requisites for running or adding new tests are the **expecttest** and **hypothesis** packages.

To run all the tests in `test_zentorch.py`:
```bash
python test/test_zentorch.py
```

To run an individual test from the test-suite you can specify `TestClassName.TestName`. Here, `TestName` is the name of the test you want to run, and `TestClassName` is the name of the class in which it is defined.
```bash
python test/test_zentorch.py TestZenTorchVersion.test_zentorch_version
```

`@parameterized.expand` works by creating new methods on the test class. To run an individual test which uses `@parameterized.expand`, new `TestName` will be generated by adding `test-number` which starts at 0 for that test and if the first parameter is a string, that string will be added to the end of the method name separated by the `_` as `TestName_test-number` or `TestName_test-number_first-parameter-string`.<br/>
For example, `supported_dtype = ["float32", "bfloat16"]` is an argument to `parameterized.expand` of test `test_zendnn_addmm_relu`.<br/>
The test case for above will generate the methods named `test_zendnn_addmm_relu_0_float32` and `test_zendnn_addmm_relu_1_bfloat16`.
```bash
python test/test_zentorch.py TestADDMM_RELU.test_zendnn_addmm_relu_0_float32
```

If you want to just run tests that contain a specific substring, you can use the `-k` flag:
```bash
python test/test_zentorch -k test_zendnn_addmm_relu
```
Above command runs all the tests which contain `test_zendnn_addmm_relu` as substring in their `TestName`.
