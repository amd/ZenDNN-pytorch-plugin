...
class Test_Qlinear(Zentorch_TestCase):
    ...
    def test_qlinear_incorrect_dtypes(
        ...
        with self.assertRaises(RuntimeError) as context:
            torch.ops.zentorch.zentorch_qlinear(
                self.data.x_for_qlinear[input_dim].to(
                    torch.bfloat16
                ),  # unsupported dtype
        ...
        with self.assertRaises(RuntimeError) as context:
            torch.ops.zentorch.zentorch_qlinear(
                ...
                self.data.bias_for_qlinear[bias_opt_idx],
                self.data.x_scales["per_tensor"],
                self.data.x_zero_points["per_tensor"][q_zero_points_dtype].to(
                    torch.int32
                ),
                self.data.y_scales[q_granularity_val],
                self.data.y_zero_points[q_granularity_val],
    ...
    def test_qlinear_incorrect_sizes(
        ...
        self.assertTrue(
            "unsupported number of elements for input_scales and/or input_zero_points "
            "with respect to input tensor" in str(context.exception)
        )
        ...
        self.assertTrue(
            ...
            "weight_zero_points with respect to weight tensor" in str(context.exception)
        )
    ...
    def test_qlinear_output_dtype_support(
        ...
        self.assertTrue(
            "output_dtype received is not yet supported, only float32 is supported"
            in str(context.exception)
        )
    ...
    def test_qlinear_accuracy(
        ...
        self.assertEqual(
            qdq_linear_output, zentorch_qlinear_output, atol=1e-5, rtol=1e-5
        )
