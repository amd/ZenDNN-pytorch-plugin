...
_embedding_bag_zendnn_impl(const at::Tensor &weight, const at::Tensor &indices,
  ...
  // check the weight type for embedding bag, only supported is fp32 for now (works ONLY for dtype=torch.float32)
  ...
  if(per_sample_weights.defined()) {
      ...
      pdesc = embedding_bag::desc(prop_kind::forward_inference,
                                  z_algorithm, ZENDNN_EMBED_BAG_THRDS,
                                  z_input.get_desc(), z_indices.get_desc(),
                                  z_offsets.get_desc(), z_weights.get_desc(),
                                  z_dst.get_desc(), padding_idx);

      pd = embedding_bag::primitive_desc(pdesc,
