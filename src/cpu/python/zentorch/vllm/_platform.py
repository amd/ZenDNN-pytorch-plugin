...
Notes:
- Out-of-tree runtime support: 0.20.0 - 0.25.1
- Legacy version map retained for shared patch/version checks: 0.15.0 - 0.25.1
...
def _create_platform():
    ...
    class ZenCPUPlatformImpl(CpuPlatform):
        ...
        Runtime-supported vLLM versions: 0.20.0 - 0.25.1.
        """
        ...
        def _patch_profiler(cls):
            """Suppress redundant cuda-time table output for OOT runtime support (v0.20.0-v0.25.1)."""
            if _is_profiler_patch_version():
                cls._patch_profiler_stop()

        @classmethod
        def _patch_profiler_stop(cls):
            """Suppress redundant cuda-time table for CPU-only OOT runtime support (v0.20.0-v0.25.1)."""
            try:
