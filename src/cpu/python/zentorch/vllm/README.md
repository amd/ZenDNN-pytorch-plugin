...
> **Note:**
> This README documents the **out-of-tree** zentorch plugin shipped by the `zentorch` package. Starting with vLLM `0.18.0`, zentorch support is also upstreamed as the native in-tree `ZenCpuPlatform`. If `zentorch` is installed on supported AMD hardware with AVX512 support, vLLM will automatically activate that in-tree platform; otherwise it falls back to the standard CPU platform. An out-of-tree compatibility warning does not necessarily mean zentorch itself is unsupported.
